...
targets = [chunk[1:]+torch.Tensor([char2idx["#"]]) for chunk in chunks]
print(f"""There are {len(examples)} chunks of 128 characters available for the
network training.""")

one_hot_examples = F.one_hot(torch.stack(examples[:-1]).long(), 128).float()
one_hot_targets = F.one_hot(torch.stack(targets[:-1]).long(), 128).float()
